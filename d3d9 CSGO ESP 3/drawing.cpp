...
void Drawespline(int bone1, int bone2, Ent* ent,D3DCOLOR color)
{
	Vec2 start, end;
	hack->WorldToScreen(hack->GetBonePos(ent, bone1),start);
	hack->WorldToScreen(hack->GetBonePos(ent, bone2), end);
	DrawLine(start.x, start.y, end.x, end.y, 3, color);
}
